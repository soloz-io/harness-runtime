            ...
            try:
                ...
                for i, event in enumerate(self.events):
                    ...
                    event_str = event_str.replace(
                        "test-job-63e8fa1b-60cb-454b-8815-96f1b4cb4574", job_id
                    )
                ...
                print(
                    f"[MOCK] Completed replaying {len(self.events)} events (with delivery buffer)"
                )
    ...
    class MockLLMModel:
        ...
        def invoke(self, messages, **kwargs):
            ...
            return AIMessage(
                ...
                response_metadata={"model": "mock-gpt-4o-mini", "finish_reason": "stop"},
            )
        ...
        def generate(self, messages_list, **kwargs):
            ...
            return LLMResult(generations=generations, llm_output={"model": "mock-gpt-4o-mini"})
...
def handle_mock_execution(execution_manager, job_id: str, trace_id: str, agent_definition: dict):
    ...
    try:
        ...
        if execution_manager and execution_manager.checkpointer:
            try:
                ...
                if checkpoints_file.exists():
                    ...
                    for i, checkpoint_data in enumerate(
                        mock_checkpoints[:3]
                    ):  # Use first 3 checkpoints
                    ...
                    logger.info(
                        "mock_checkpoints_created",
                        job_id=job_id,
                        trace_id=trace_id,
                        count=min(3, len(mock_checkpoints)),
                    )
                else:
                    logger.warning(
                        "mock_checkpoints_file_not_found",
                        job_id=job_id,
                        trace_id=trace_id,
                ...
                logger.warning(
                    "mock_checkpoint_creation_failed",
                    job_id=job_id,
                    trace_id=trace_id,
                    error=str(e),
                )
        ...
        logger.warning(
            "mock_execution_setup_failed", job_id=job_id, trace_id=trace_id, error=str(e)
        )
