...
def validate_minimum_events(
    events: List[Dict[str, Any]], use_typical: bool = True
) -> Tuple[bool, List[str]]:
    """
...
def validate_specialist_order(events: List[Dict[str, Any]]) -> Tuple[bool, List[str]]:
    ...
    last_state_update = next(
        (e for e in reversed(events) if e.get("event_type") == "on_state_update"), None
    )

    if not last_state_update:
        errors.append(
            "Validation Error: No 'on_state_update' events found to validate specialist order."
        )
        return False, errors
...
def validate_event_structure(events: List[Dict[str, Any]]) -> Tuple[bool, List[str]]:
    ...
    if end_events:
        ...
        if last_end != len(events) - 1:
            errors.append(
                f"'end' event should be last, but found at position {last_end} of {len(events)}"
            )
...
def extract_checkpoints(
    postgres_connection: psycopg.Connection, job_id: str
) -> List[Dict[str, Any]]:
    """
...
def validate_workflow_result(
    result: Dict[str, Any], checkpoints: List[Dict[str, Any]]
) -> Tuple[bool, List[str]]:
    """
...
def validate_redis_artifacts(events: List[Dict[str, Any]], job_id: str) -> Tuple[bool, List[str]]:
    ...
    # ================================================================
    if (
        "/definition.json" in files_state and not errors
    ):  # Only validate if file exists and no previous errors
        try:
            # Load the schema
...
def validate_checkpoint_artifacts(
    checkpoints: List[Dict[str, Any]], job_id: str
) -> Tuple[bool, List[str]]:
    """
...
def generate_execution_summary(
    ...
    for spec in specialist_timeline:
        summary_lines.append(f"  Step {spec['step']}: {spec['event_type']} at {spec['timestamp']}")
...
def generate_checkpoint_summary(checkpoints: List[Dict[str, Any]]) -> str:
    ...
    if len(checkpoints) <= 20:
        ...
        summary_lines.extend(
            ["Checkpoint Timeline (showing first 5 and last 5):", "First 5 checkpoints:"]
        )
...
def extract_and_save_generated_files(
    events: List[Dict[str, Any]], run_dir: Path = None
) -> Dict[str, str]:
    """
    ...
    for file_path, content in extracted_files.items():
        ...
        if not any(
            safe_filename.endswith(ext) for ext in [".md", ".json", ".py", ".txt", ".yaml", ".yml"]
        ):
            safe_filename += ".txt"
    ...
    manifest = {"total_files": len(extracted_files), "files": []}
                ...
                manifest["files"].append(
                    {
                        ...
                        else len(str(file_data.get("content", ""))),
                    }
                )
